...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        print("\n========== Model.__init__ ==========")
        print(f"[CONFIG] use_stformer={getattr(configs, 'use_stformer', False)}")
    ...
    def forward_train(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        """
        # 只在第一个 batch 打印详细维度
        if not hasattr(self, '_print_count') or self._print_count == 0:
            self._print_count = 0
            print("\n========== forward_train (first batch dims) ==========")
            print(f"[INPUT] x_enc: {x_enc.shape}, x_dec: {x_dec.shape}")
    ...
    def forward_val_test(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        if self.configs.new_norm:
            ...
            x_past = x_past_normed.permute(0, 2, 1)
        else:
