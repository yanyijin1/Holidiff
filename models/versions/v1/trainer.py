...
def l1loss(pred, target):
    loss = (target - pred).abs().mean(dim=1).mean(dim=1).mean()
    return loss
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                f_dim = -1 if self.args.features == 'MS' else 0
                outputs = outputs[:, f_dim:, -self.args.pred_len:]
                batch_y = batch_y[:, -self.args.pred_len:, f_dim:].to(self.device)
                
                if self.args.is_diff and self.args.loss_type == 'MAE':
                    loss = criterion(outputs / weight_tmp, batch_y.permute(0, 2, 1) / weight_tmp)
                else:
