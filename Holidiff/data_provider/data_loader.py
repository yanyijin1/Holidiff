...
from Holidiff.utils.timefeatures import time_features
from Holidiff.data_provider.m4 import M4Dataset, M4Meta
from Holidiff.data_provider.uea import subsample, interpolate_missing, Normalizer
try:
...
import warnings
try:
    from Holidiff.utils.augmentation import run_augmentation_single
except Exception:
...
class Dataset_Production(Dataset):
    ...
    def __read_data__(self):
        ...
        cols.remove('date')
        df_raw = df_raw[['date'] + cols + [self.target]]
        num_train = int(len(df_raw) * 0.907)
        num_test = int(len(df_raw) * 0.032)
        num_vali = len(df_raw) - num_train - num_test
        border1s = [0, num_train - self.seq_len, len(df_raw) - num_test - self.seq_len]
        border2s = [num_train, num_train + num_vali, len(df_raw)]
        border1 = border1s[self.set_type]
...
class Dataset_Caiso(Dataset):
    ...
    def __read_data__(self):
        ...
        cols.remove('date')
        df_raw = df_raw[['date'] + cols + [self.target]]
        num_train = int(len(df_raw) * 0.8123)
        num_test = int(len(df_raw) * 0.0870)
        num_vali = len(df_raw) - num_train - num_test
        border1s = [0, num_train - self.seq_len, len(df_raw) - num_test - self.seq_len]
        border2s = [num_train, num_train + num_vali, len(df_raw)]
        border1 = border1s[self.set_type]
