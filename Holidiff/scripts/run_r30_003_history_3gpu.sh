...
RUN_ROOT="/root/autodl-tmp/STdiff_runs/history_boundary_r30_003"
LAUNCH_LOG_ROOT="/root/autodl-tmp/STdiff_runs/launch_logs/r30_003_3gpu"
...
DATASETS=(PEMS03-R30-003 PEMS04-R30-003 PEMS08-R30-003)
GPUS=(0 1 2)
    ...
    for hr in "${HISTORY_RATIOS[@]}"; do
      ...
      for model in "${MODELS[@]}"; do
        ...
        CUDA_VISIBLE_DEVICES="${gpu}" \
          /root/miniconda3/bin/conda run -n ${CONDA_ENV} python ${ROOT_DIR}/train.py \
            --config    "${config_path}" \
            --version   "${run_name}" \
            --model     "${model_name}" \
            --model_id  "${model_id}" \
...
PID_FILE="${LAUNCH_LOG_ROOT}/r30_003_3gpu_ep${TRAIN_EPOCHS}_seed${SEED}.pids"
printf '%s\n' "${PIDS[@]}" > "${PID_FILE}"
