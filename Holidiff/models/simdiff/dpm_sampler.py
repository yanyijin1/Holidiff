...
class DPMSolverSampler(object):
    def __init__(self, args,model, df_device,df_alphas_cumprod,df_betas_device, **kwargs):
        ...
        lower_order_final = getattr(args, 'lower_order_final', True)
        if isinstance(lower_order_final, str):
            self.lower_order_final = lower_order_final.lower() == 'true'
        else:
    ...
    def register_buffer(self, name, attr):
        if type(attr) == torch.Tensor:
            target_device = torch.device(self.df_device)
